# -*- coding: utf-8 -*-
"""
Created on Fri Jun 16 19:56:27 2017
...
def train(modelclass, dt=None):
    ...
    X_train, y_train, X_valid, y_valid = Mnist()  # 0617_1346: 0.05   #0619_1033: 0.01   0619_1528:0.1
    ...
    with gan.tf_graph.as_default():
        ...
        for epoch in range(1, max_epoch):
            ...
            for X_batch, y_batch in data_train:
                ...
                # import pdb; pdb.set_trace()
                fake_judge_v, cost_train, gen_cost = gan.sess.run([fake_judge, cost_train_mg, gen_train_cost_sb],  #  , dis_cost =  , dis_train_cost_sb
                                                               feed_dict=feed_dict)
                                                               
                                                               
                if batch_iter == 1:
                    G_train, G_img = gan.sess.run([G_train_sb, gen_train_mg], feed_dict=feed_dict)
                    gen_writer = tf.summary.FileWriter('{}/generator/{}'.format(logdir, epoch))
