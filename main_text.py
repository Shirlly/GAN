# -*- coding: utf-8 -*-
"""
Created on Tue May 30 17:19:26 2017
...
from data_text import data_char
...
def text_embed(ch_embed_dim, sent_len, word_len):
    
    X_ph = tf.placeholder('int32', [None, sent_len, word_len])
    start = StartNode(input_vars=[X_ph])
    
    # character CNN
    embed = HiddenNode(prev=[start], layers=[Reshape(shape=(-1, word_len)),
                                               Embedding(cat_dim=unicode_size,
                                                         encode_dim=ch_embed_dim,
                                                         zero_pad=True),
                                               Reshape(shape=(-1, ch_embed_dim, word_len, sent_len))])
    end_nodes = [EndNode(prev=[embed])]
    graph = Graph(start=[start,startw], end=end_nodes)
    train_embed = graph.train_fprop()
    valid_embed = graph.test_fprop()
    
    return train_embed, valid_embed
